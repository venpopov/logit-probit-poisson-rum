...
The present work addresses this by shifting perspective. Rather than beginning with static assumptions about utility noise, this paper begins with a stochastic accumulation process and asks what class of random utility models it induces. A related line of work is the dependent Poisson race model of @Ruan2008, who develop a thresholded accumulation process for binary conjoint choice. Their focus, however, is on modeling dependence between alternatives via shared Poisson components, rather than on the distributional consequences of thresholded accumulation for the logit–probit relationship. The present contribution is complementary: by restricting attention to independent races and introducing a variance-preserving identification, it derives both multinomial logit and multinomial probit as endpoint regimes of a single generative family.
...
Within the temperature-identified Poisson count race, two classical models emerge as limiting cases of a single generative mechanism. When $\theta = 1$, the stochastic utility component is exactly Gumbel distributed, yielding the **Multinomial Logit** model without approximation. As $\theta \to \infty$, the standardized log-Gamma noise converges in distribution to a Gaussian by the Central Limit Theorem, and the resulting choice probabilities converge to those of the **Multinomial Probit** model. From this perspective, the familiar distinction between logit and probit reflects differences in the stopping rule governing an evidence accumulation process, rather than fundamentally distinct assumptions about decision noise.

Specifically, the paper establishes three results: (1) Multinomial Logit arises exactly as the single-event threshold case of a Poisson race; (2) Multinomial Probit emerges as a non-degenerate asymptotic limit under temperature identification; and (3) intermediate accumulation thresholds correspond to log-Gamma random utility models with a clear process interpretation. The remainder of the paper formalizes this framework, presents simulations illustrating the interpolation between regimes, and discusses implications for discrete choice modeling.
